...
# 3. Launch Alpine container with Network Administration capabilities (--cap-add=NET_ADMIN)
...
docker exec -u snowy $CONTAINER_NAME pip install --user --break-system-packages selenium pwinput
...
docker exec -u root $CONTAINER_NAME sh -c "
    ...
    # Create a dynamic named set for our whitelisted IPs
    nft add set inet filter allowed_ips { type ipv4_addr \; }
    nft add rule inet filter output ip daddr @allowed_ips accept

    # Add initial resolved IPs to the set
    nft add element inet filter allowed_ips { $JUST_DICE_IP, $CF_INSIGHTS_IP }
"
